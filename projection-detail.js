...
(function(MH){
  ...
  function contribution(ws,y){
    ...
    if(ws===current){
      const elapsed=C.weekDays(ws).filter(k=>k<=today).reduce((n,k)=>n+D.forecastMinutes(k),0);
      return D.weekAccounted(ws)+Math.max(0,D.weekForecast(ws)-elapsed);
    }
    if(ws<current)return D.weekAccounted(ws);
    return D.weekForecast(ws);
  }

  function rowsFor(y){
    const current=C.weekStart(C.dateKey());
    return C.mondays(y).map(ws=>{
      const projected=contribution(ws,y), objective=D.weeklyObjective(ws);
  ...
  function inject(){
    ...
    const summary=view.querySelector('.annual-summary');
    const yearInput=view.querySelector('#forecastYear');
    if(!summary||!yearInput)return;
    view.querySelector('#projectionDetail')?.remove();
    const y=Number(yearInput.value), currentYear=Number(C.dateKey().slice(0,4));
    const closing=D.openingAtYear(y)==null?null:D.openingAtYear(y)+D.annualProjection(y)-D.annualObjective(y);
    ...
    box.id='projectionDetail';
    box.className='projection-detail';
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('[data-view="forecastView"], [data-action="switch-view"]'))setTimeout(inject,0);
  });
