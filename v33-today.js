/* v33: heure de depart indicative pour atteindre la duree prevue du jour */
(function(){
...
function targetToday(k){const override=window.dayForecastOverrideV32?.(k);return override==null?Number(expectedFor(k)||0):Number(override||0)}
function departureCard(){const k=dateKey(),day=state.days[k],open=openSegment(day);if(!day||!open?.start)return '';const target=targetToday(k);if(target<=0)return '';const requiredGross=target+pauseFor(day),done=grossDoneBeforeOpen(day),remainingGross=Math.max(0,requiredGross-done),departure=mins(open.start)+remainingGross,weekRemain=Math.max(0,weekTarget(k)-weekWorked(k));return `<div class="card departure-v33"><h2>Repère de fin de journée</h2><div class="row"><span>Pour atteindre ${fmtPlainMinutes(target)} aujourd’hui</span><strong>${hhmmFromMinutes(departure)}</strong></div><p class="muted compact">Départ indicatif avec ${pauseFor(day)} min de pause. Si la pause change, l’heure se recalcule.</p><div class="time-row row"><span>Reste pour l’objectif de la semaine</span><strong>${fmtPlainMinutes(weekRemain)}</strong></div></div>`}
