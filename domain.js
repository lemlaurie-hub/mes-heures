...
function remainingWorkForPart(event,base){const part=event.part||'full';if(part==='full')return 0;const split=clock(event.splitTime||(part==='morning'?'13:30':'12:00')),a=clock(base.start),b=clock(base.end);if(split==null||a==null||b==null)return Math.round(base.minutes/2);if(part==='morning')return Math.max(0,b-Math.max(a,split));return Math.max(0,Math.min(b,split)-a)}
function forecastMinutes(k,pending=false){const base=baseDay(k);let m=base.minutes;for(const e of eventsFor(k,pending)){if(e.type!=='Récup'||e.status!=='validated')continue;m=Math.min(m,remainingWorkForPart(e,base))}return Math.max(0,Math.round(m))}
function weekForecast(ws,pending=false){return weekDays(ws).reduce((n,k)=>n+forecastMinutes(k,pending),0)}
function neutralCredit(k){const base=baseDay(k),neutral=['Congé','Congé exceptionnel','Arrêt maladie','Férié'];let credit=0;for(const e of eventsFor(k)){if(!neutral.includes(e.type))continue;if((e.part||'full')==='full')credit=Math.max(credit,base.minutes);else credit=Math.max(credit,Math.max(0,base.minutes-remainingWorkForPart(e,base)))}return Math.round(credit)}
...
function assignPlanningRange(profileId,start,end,status='validated'){normalizePlanningPeriods();if(!start||!end||end<start)throw new Error('Période invalide');const principal=principalPlan(),next=[];for(const p of cfg().planningPeriods){if(p.end<start||p.start>end){next.push(p);continue}if(p.start<start)next.push({...p,id:uid('per'),end:addDays(start,-1)});if(p.end>end)next.push({...p,id:uid('per'),start:addDays(end,1)})}if(profileId&&profileId!==principal?.id)next.push({id:uid('per'),profileId,start,end,status,createdAt:dateKey()});cfg().planningPeriods=mergePeriods(next);save();return cfg().planningPeriods}
...
function saveEvent(data,id=''){cfg().futureEvents=Array.isArray(cfg().futureEvents)?cfg().futureEvents:[];let e=id?cfg().futureEvents.find(x=>x.id===id):null;if(e){Object.assign(e,data,{updatedAt:dateKey()})}else{e={id:uid('evt'),createdAt:dateKey(),...data};cfg().futureEvents.push(e)}if(e.status==='validated'&&!e.validatedAt)e.validatedAt=dateKey();if(e.status!=='validated')e.validatedAt='';save();return e}
function createEvent(data){return saveEvent(data)}
...
function workedRetainedForDay(k){const day=state.days[k],w=day?worked(day):null,manual=Math.max(0,Number(day?.retainedMinutes||0)),neutral=neutralCredit(k);return Math.max(0,Number(w||0))+manual+neutral}
function currentWeekWorked(k=dateKey()){return weekDays(k).reduce((n,d)=>n+workedRetainedForDay(d),0)}
function currentBalance(k=dateKey()){let total=Number(state.initialBalanceMinutes||0)+(state.historicalWeeks||[]).reduce((n,w)=>n+Number(w.deltaMinutes||0),0);const ws=weekStart(k),today=dateKey();if(ws!==weekStart(today))return total;for(const d of weekDays(ws)){if(d>=today)break;total+=workedRetainedForDay(d)-forecastMinutes(d)}return Math.round(total)}
window.MH={plans,planBy,typeBy,principalPlan,planDays,activePeriod,activePlan,plannedType,baseDay,eventsFor,forecastMinutes,weekForecast,neutralCredit,assignPlanningRange,removePlanningPeriod,saveDayOverride,removeDayOverride,saveEvent,createEvent,validateEvent,removeEvent,weeklyObjective,currentWeekWorked,currentBalance};
...
weekWorked=k=>currentWeekWorked(k);
