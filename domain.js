...
(function(MH){
  ...
  function weekForecast(ws,pending=false){return C.weekDays(ws).reduce((n,k)=>n+forecastMinutes(k,pending),0)}
  ...
  function weekAccounted(ws=C.weekStart()){const hist=(S().historicalWeeks||[]).find(w=>w.weekStart===C.weekStart(ws));if(hist)return Number(hist.workedMinutes||0);return C.weekDays(ws).reduce((n,k)=>n+accountedMinutes(k),0)}
  ...
  function annualProjection(y){const today=C.dateKey(),hist=new Map((S().historicalWeeks||[]).map(w=>[w.weekStart,w]));let n=0;for(const ws of C.mondays(y)){const h=hist.get(ws);if(h){n+=Number(h.workedMinutes||0);continue}if(ws===C.weekStart(today)){n+=weekAccounted(ws)+Math.max(0,weekForecast(ws)-C.weekDays(ws).filter(k=>k<=today).reduce((s,k)=>s+forecastMinutes(k),0));continue}if(ws<C.weekStart(today)){n+=weekAccounted(ws);continue}n+=weekForecast(ws)}return Math.round(n)}
  ...
  MH.domain={NEUTRAL_TYPES,plans,planBy,typeBy,principalPlan,planDays,activePeriod,activePlan,plannedType,baseDay,holidays,holidayFor,storedEventsFor,eventsFor,dayExpectation,actualPause,actualWorked,actualDayState,forecastMinutes,requiredWorkMinutes,accountedMinutes,weekForecast,scheduleFor,weeklyObjective,weekAccounted,weekActualWorked,weekReconstruction,replaceConsolidatedWeek,remainingThisWeek,paidForWeek,reference,annualReference,balanceFromReference,historicalCarryBefore,currentWeekDelta,currentBalance,projectedWeekEndBalance,openingAtYear,annualObjective,annualProjection,projectedClosingAtYear,annualRealized,historyWeekRows,weekActualState,weekInfo,assignPlanningRange,removePlanningPeriod,saveDayOverride,removeDayOverride,upsertEvent,removeEvent,setEventRange,validateEvent,saveDayActual,saveDayType,deleteDayType,createPlanning,savePlanningVersion,archivePlanning,saveProfile,saveAlerts,thresholdStatus,departureEstimate};
})(window.MH);
